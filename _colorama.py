...
def remove_color(string: str) -> str:
    ...
    # 2. escape them and build a regular expression that searches for everything but them
    escaped_formatting_sequence_list = [re.escape(item) for item in formatting_sequence_list]
    not_formatting_sequence_regex = re.compile("[^{}]".format(''.join(escaped_formatting_sequence_list)))
